...
class LoginRequest extends FormRequest
{
    ...
    {
        if ($this->login_field == 'email_username') {
            $this->login_field = filter_var($this->input('email_username'),FILTER_VALIDATE_EMAIL) ? 'email' : 'username';
            $this->merge([$this->login_field => $this->input('email_username')]);
        }
    }
    ...
    {
        return Str::lower($this->input($this->login_field)).'|'.$this->ip();
    }
}
